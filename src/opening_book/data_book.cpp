...
size_t const last_element = sorted_variation_collection.size() - 1;
assert(sorted_variation_collection.size() >= minimum_book_size_for_lookups_and_test_of_sortedness);
...
size_t const first_index = first_matching_index(moves_from_startpos_in_uci_format);
if (first_index == VARIATION_NOT_FOUND) {
return VARIATION_NOT_FOUND;
}
size_t const last_index = last_matching_index(moves_from_startpos_in_uci_format);
assert(last_index != VARIATION_NOT_FOUND);
assert(last_index >= first_index);
assert(last_index < sorted_variation_collection.size());
